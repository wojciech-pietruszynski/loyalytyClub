    ...
    <div className="card" style={{ maxWidth: '500px', margin: '0 auto' }}>
      ...
      <form onSubmit={(event) => { void handleAddPoints(event); }}>
        <div className="form-group">
          ...
          <select className="input" value={newPoints.customerId} onChange={(event) => setNewPoints((prev) => ({ ...prev, customerId: event.target.value }))} required>
        ...
        <div className="form-group"><label>{t('pointsCount')}</label><input className="input" type="number" value={newPoints.points} onChange={(event) => setNewPoints((prev) => ({ ...prev, points: parseInt(event.target.value, 10) || 0 }))} required /></div>
        <div className="form-group"><label>{t('description')}</label><input className="input" value={newPoints.description} onChange={(event) => setNewPoints((prev) => ({ ...prev, description: event.target.value }))} required /></div>
